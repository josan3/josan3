<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:050505,50:071a12,100:00ff88&height=200&section=header&text=JOSÉ%20ANTONIO&fontSize=46&fontColor=00ff88&animation=fadeIn&fontAlignY=35&desc=MONTES%20SOLANO&descAlignY=58&descSize=17&descColor=e6edf3" width="100%"/>

<a href="https://readme-typing-svg.demolab.com/">
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=21&pause=1200&color=00FF88&center=true&vCenter=true&width=800&lines=Cybersecurity+Engineer;Web+Application+Security;Penetration+Testing;Security+Automation;SIEM+%26+Security+Monitoring" alt="Typing SVG"/>
</a>

<br><br>

<a href="https://www.linkedin.com/in/jos%C3%A9-antonio-montes-solano-ab514b37b/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>

<a href="https://github.com/josan3">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

</div>

---

## 🛡️ About Me

I'm a **Cybersecurity Engineer** focused on both offensive and defensive security, with a strong interest in **web application security, penetration testing, security automation and security monitoring**.

I enjoy building practical cybersecurity projects, automating repetitive security tasks and turning security concepts into working solutions.

```text
┌──────────────────────────────────────────────┐
│              CYBERSECURITY FOCUS             │
├──────────────────────────────────────────────┤
│                                              │
│  🔴 Offensive Security                       │
│  🔵 Defensive Security                       │
│  🟢 Security Engineering                     │
│                                              │
└──────────────────────────────────────────────┘
```

---

# ⚔️ Security Projects

## 🔎 AnalizadorWeb

**AI-assisted platform for automated web penetration testing.**

A cybersecurity project designed to support different stages of the web security assessment workflow.

```text
RECON
  ↓
ANALYSIS
  ↓
FINDINGS
  ↓
RISK
  ↓
REPORT
```

**Features**

`Reconnaissance` · `Security Analysis` · `Evidence Collection`
`Finding Normalization` · `Risk Prioritization` · `AI` · `Python`

**→ [View Project](https://github.com/josan3/AnalizadorWeb)**

---

## 🛡️ SIEM

**Security Information & Event Management project.**

Focused on security event collection, log analysis and detection.

```text
LOGS
  ↓
COLLECTION
  ↓
ANALYSIS
  ↓
DETECTION
  ↓
ALERT
```

**Features**

`Security Events` · `Log Analysis` · `Detection` · `Python`

**→ [View Project](https://github.com/josan3/SIEM)**

---

## 🏥 Nurse in the Pocket

**Progressive Web App developed as a Bachelor's Thesis for healthcare management and patient monitoring.**

```text
FRONTEND
  ↓
API
  ↓
DATABASE
  ↓
AUTHENTICATION
  ↓
DEPLOYMENT
```

**Technologies**

`React` · `PWA` · `Node.js` · `MySQL`
`Firebase` · `Jenkins` · `Postman`

**→ [View Project](https://github.com/josan3/Nurseinthepocket)**

---

## 🚁 Drones

**Java software development project carried out during an international project in Germany.**

```text
REQUIREMENTS
  ↓
DEVELOPMENT
  ↓
TESTING
  ↓
TEAMWORK
```

**Technologies**

`Java` · `Software Development` · `Teamwork`
`International Collaboration`

**→ [View Project](https://github.com/josan3/Drones)**

---

# 💻 Tech Stack

<div align="center">

### 🔐 Cybersecurity

<img src="https://skillicons.dev/icons?i=python,linux,bash,git" />

<br><br>

`Pentesting` · `Web Security` · `Vulnerability Assessment`
`SIEM` · `Security Monitoring` · `Security Automation`

<br><br>

### 🧑‍💻 Development

<img src="https://skillicons.dev/icons?i=java,js,cpp,react,nodejs,mysql" />

<br><br>

### ☁️ Tools & Platforms

<img src="https://skillicons.dev/icons?i=firebase,jenkins,postman,github" />

</div>

---


<div align="center">

```text
$ echo "Learn. Build. Test. Secure."
```

### `Cybersecurity Engineer`

**Pentesting · Web Security · Security Automation · SIEM**

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00ff88,50:071a12,100:050505&height=100&section=footer" width="100%"/>

